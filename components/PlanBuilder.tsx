  ...
  return (
    <div className="prose prose-sm sm:prose lg:prose-lg xl:prose-xl max-w-none">
      {lines.map((line, index) => {
        if (line.startsWith('## ')) return <h2 key={index} className="text-2xl font-bold text-navy-900 mt-6 mb-3 border-b pb-2">{line.substring(3)}</h2>;
        if (line.startsWith('### ')) return <h3 key={index} className="text-xl font-semibold text-navy-800 mt-4 mb-2">{line.substring(4)}</h3>;
        if (line.startsWith('* ')) return <li key={index} className="ml-6">{line.substring(2)}</li>;
        if (line.trim() === '') return null;

        const parts = line.split(/(\*\*.*?\*\*)/g).filter(part => part);
        return (
          <p key={index} className="my-2 text-navy-800">
            {parts.map((part, i) =>
...
const PlanBuilder: React.FC = () => {
    ...
    const ProgressIndicator = () => (
        <div className="mb-6">
            <div className="flex justify-between mb-1">
                <span className="text-base font-medium text-usace-blue">Step {currentStep} of 10</span>
                <span className="text-sm font-medium text-usace-blue">{currentStep * 10}%</span>
            </div>
    ...
    if (isLoading) {
        return (
            <div className="bg-white p-6 rounded-lg shadow-md h-full flex flex-col items-center justify-center">
                <h2 className="text-2xl font-bold text-navy-900 mb-4">Generating Your Plan...</h2>
                <p className="text-gray-600 mb-6">The AI is synthesizing your inputs into a professional document.</p>
    ...
    if (currentStep === 12) {
        return (
             <div className="bg-white p-6 rounded-lg shadow-md h-full flex flex-col">
                <h2 className="text-3xl font-bold text-navy-900 mb-4">Your Communication Plan</h2>
                <div className="flex-1 overflow-y-auto mb-4 p-4 bg-navy-50 rounded-md border">
                    <RenderMarkdown content={generatedPlan} />
                </div>
                <div className="flex justify-end space-x-4">
                    <button onClick={handleStartOver} className="inline-flex justify-center rounded-md border border-gray-300 bg-white py-2 px-4 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                        Start Over
                    </button>
                    <button onClick={() => navigator.clipboard.writeText(generatedPlan)} className="inline-flex justify-center rounded-md border border-transparent bg-usace-blue py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-navy-800">
                        Copy to Clipboard
                    </button>
                </div>
            </div>
        ...
        return (
            <div className="bg-white p-6 rounded-lg shadow-md h-full flex flex-col">
                ...
                <div className="flex-1 flex flex-col">
                    <h3 className="text-2xl font-bold text-navy-900 mb-2">{STEP_PROMPTS[currentStep].title}</h3>
                    <p className="text-gray-600 mb-4">{STEP_PROMPTS[currentStep].prompt}</p>
                    <textarea 
                        value={currentInput}
                        onChange={(e) => setCurrentInput(e.target.value)}
                        className="flex-1 w-full p-2 border border-gray-300 rounded-md resize-none focus:ring-usace-blue focus:border-usace-blue"
                        placeholder="Your notes here..."
                    />
                </div>
                <div className="flex justify-between mt-6">
                    <button onClick={handlePrevious} disabled={currentStep <= 1} className="inline-flex justify-center rounded-md border border-gray-300 bg-white py-2 px-4 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed">
                        Previous
                    </button>
                    {currentStep < 10 ? (
                        <button onClick={handleNext} className="inline-flex justify-center rounded-md border border-transparent bg-usace-blue py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-navy-800">
                            Next Step
                        </button>
                    ) : (
                         <button onClick={handleGeneratePlan} className="inline-flex items-center justify-center rounded-md border border-transparent bg-usace-red py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-red-700">
    ...
    return (
         <div className="bg-white p-8 rounded-lg shadow-md h-full flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-3xl font-bold text-navy-900 mb-4">AI Communication Plan Builder</h2>
            ...
            <button onClick={handleStart} className="inline-flex justify-center rounded-md border border-transparent bg-usace-blue py-3 px-6 text-base font-medium text-white shadow-sm hover:bg-navy-800">
                Let's Get Started
            </button>
        </div>
    );
